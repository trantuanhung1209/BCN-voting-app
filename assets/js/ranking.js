    ...
    return new Promise((resolve, reject) => {
        ...
        onValue(countRef, (snapshot) => {
            ...
            snapshot.forEach((item) => {
                const data = item.val();
                userCounts.push(data.usersVote);
            });

            // Tính tổng số lượt vote
            let totalVote = userCounts.length;
            console.log(`quantity of vote: ${totalVote}`);

            updateRanking(totalVote); // Truyền totalVote vào updateRanking
        });
    });
};


const updateRanking = (totalVote) => { // Nhận totalVote từ getCount
    onValue(performancesRef, (snapshot) => {
        ...
        const htmls = groupedNames.map(item => {
            const percentage = (((item.count) / totalVote) * 100).toFixed(2) >= 100 ? 100 : (((item.count) / totalVote) * 100).toFixed(2);
            return `
        ...
        document.querySelectorAll('.performer-item').forEach((item) => {
            ...
            if (oldPosition !== undefined) {
                anime({
                    targets: item,
                    translateY: [oldPosition - newPosition, 0],
                    duration: 1000,
                    easing: 'easeOutExpo'
                });
            } else {
